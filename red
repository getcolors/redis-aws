...
// own: validation, the graph and the steps all live in the `package-redis-red`
...
//
// "package-redis-red" is null before this repository has been pushed and
...
// rewrites null to "github:getcolors/redis#<sha>" in place.
//
...
// add-deps. REDIS_LIB_ROOT overrides everything with a working tree: point it
 ...
 *
 * The payload's canonical home is skills/package-redis-red/ inside the redis
...
function checkoutRedDir() {
  ...
  return manifest?.name === "package-redis-red-dev" ? candidate : null;
}
...
function installToCache() {
  ...
  const root = join(home, "package-redis-red");
  ...
  writeFileSync(
    ...
    `${JSON.stringify({ name: "package-redis-red-cache", private: true, dependencies: PINS }, null, 2)}\n`,
  );
...
let redis;
try {
  redis = await import("package-redis-red");
} catch (err) {
  ...
  const libRoot = process.env.REDIS_LIB_ROOT;
  ...
  if (libRoot) {
    ...
    redis = await importWorkingTree(overrideDir, "REDIS_LIB_ROOT");
  ...
  } else if (!PINS["package-redis-red"]) {
    console.error(
    ...
    // not to a path.
    redis = await import(Bun.resolveSync("package-redis-red", dir));
  }
}

export const run = redis.run;
if (import.meta.main) await redis.exec();
